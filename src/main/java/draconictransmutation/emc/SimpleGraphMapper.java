...
import draconictransmutation.emc.collector.MappingCollector;
...
public class SimpleGraphMapper<T, V extends Comparable<V>, A extends IValueArithmetic<V>> extends MappingCollector<T, V, A> implements IValueGenerator<T, V> {
			...
			for (Map.Entry<T, Set<Conversion>> entry : conversionsFor.entrySet()) {
				...
				for (Conversion conversion : entry.getValue()) {
					...
					if (ZERO.compareTo(ingredientValue) < 0 && resultValueConversion.compareTo(resultValueActual) < 0) {
						if (overwriteConversion.containsKey(conversion.output) && overwriteConversion.get(conversion.output) != conversion) {
							if (logFoundExploits) {
								DTCore.LOGGER.warn("EMC Exploit: \"{}\" ingredient cost: {} value of result: {} setValueFromConversion: {}", conversion, ingredientValue, resultValueActual, overwriteConversion.get(conversion.output));
							}
							...
							DTCore.LOGGER.warn("EMC Exploit: ingredients ({}) cost {} but output value is {}", conversion, ingredientValue, resultValueActual);
						}
					}
				}
	...
	private V valueForConversion(Map<T, V> values, Conversion conversion) {
		try {
			...
			DTCore.LOGGER.warn("Could not calculate value for {}: {}", conversion.toString(), e.toString());
			return ZERO;
		} catch (Exception e) {
			DTCore.LOGGER.warn("Could not calculate value for {}: {}", conversion.toString(), e.toString(), e);
			return ZERO;
		}
	}
