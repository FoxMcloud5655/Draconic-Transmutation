...
public class DumpMissingEmc {
	...
	private static int execute(CommandContext<CommandSource> ctx) {
		...
		if (missingCount == 0) {
			source.sendFeedback(DTLang.DUMP_MISSING_EMC_NONE_MISSING.translate(), true);
		} else {
			if (missingCount == 1) {
				source.sendFeedback(DTLang.DUMP_MISSING_EMC_ONE_MISSING.translate(), true);
			} else {
				source.sendFeedback(DTLang.DUMP_MISSING_EMC_MULTIPLE_MISSING.translate(missingCount), true);
			}
			for (ItemInfo itemInfo : missing) {
				DTCore.LOGGER.info(itemInfo);
			}
		}
		return missingCount;
	}
