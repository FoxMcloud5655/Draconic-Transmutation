...
public class AnnotationHelper {
			...
			for (AnnotationData data : scanData.getAnnotations()) {
				if (NBT_PROCESSOR_TYPE.equals(data.getAnnotationType()) && checkRequiredMods(data)) {
					...
					if (processor != null) {
						...
						DTCore.LOGGER.info("Found and loaded NBT Processor: {}, with priority {}", processor.getName(), priority);
					}
				}
			}
		}
			...
			for (AnnotationData data : scanData.getAnnotations()) {
				if (RECIPE_TYPE_MAPPER_TYPE.equals(data.getAnnotationType()) && checkRequiredMods(data)) {
					...
					if (mapper != null) {
						...
						DTCore.LOGGER.info("Found and loaded RecipeType Mapper: {}, with priority {}", mapper.getName(), priority);
					}
				}
			}
		}
				...
				if (MAPPER_TYPE.equals(data.getAnnotationType()) && checkRequiredMods(data)) {
					...
					if (mapper != null) {
						try {
							...
							DTCore.LOGGER.info("Found and loaded EMC mapper: {}, with priority {}", mapper.getName(), priority);
						} catch (ClassCastException e) {
							DTCore.LOGGER.error("{}: Is not a mapper for {}, to {}", mapper.getClass(), NormalizedSimpleStack.class, Long.class, e);
						}
					}
				}
			}
	...
	private static <T> T createOrGetInstance(String className, Class<T> baseClass, Class<? extends Annotation> instanceAnnotation, Function<T, String> nameFunction) {
		...
		try {
					...
					if (Modifier.isStatic(field.getModifiers())) {
						try {
							...
							if (baseClass.isInstance(fieldValue)) {
								...
								DTCore.debugLog("Found specified {} instance for: {}. Using it rather than creating a new instance.", baseClass.getSimpleName(),
										nameFunction.apply(instance));
								return instance;
							} else {
								DTCore.LOGGER.error("{} annotation found on non {} field: {}", instanceAnnotation.getSimpleName(), baseClass.getSimpleName(), field);
								return null;
							}
						} catch (IllegalAccessException e) {
							DTCore.LOGGER.error("{} annotation found on inaccessible field: {}", instanceAnnotation.getSimpleName(), field);
							return null;
						}
					} else {
						DTCore.LOGGER.error("{} annotation found on non static field: {}", instanceAnnotation.getSimpleName(), field);
						return null;
					}
				}
			}
			...
			DTCore.LOGGER.error("Failed to load: {}", className, e);
		}
		return null;
	}
	...
	private static boolean checkRequiredMods(AnnotationData data) {
		...
		if (annotationData.containsKey("requiredMods")) {
			...
			if (requiredMods.stream().anyMatch(modid -> !ModList.get().isLoaded(modid))) {
				DTCore.debugLog("Skipped checking class {}, as its required mods ({}) are not loaded.", data.getMemberName(), Arrays.toString(requiredMods.toArray()));
				return false;
			}
		}
		return true;
