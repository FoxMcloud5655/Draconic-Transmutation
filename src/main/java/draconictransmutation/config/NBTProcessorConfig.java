 ...
 */
public class NBTProcessorConfig extends BaseDTConfig {
	...
	public static void setup(@Nonnull List<INBTProcessor> processors) {
		if (INSTANCE == null) {
			DTConfig.registerConfig(INSTANCE = new NBTProcessorConfig(processors));
		}
	}
	...
	public static boolean isEnabled(INBTProcessor processor) {
		...
		if (processorConfig == null) {
			DTCore.LOGGER.warn("Processor Config: '{}' is missing from the config.", name);
			return false;
		}
	...
	public static boolean hasPersistent(INBTProcessor processor) {
		...
		if (processorConfig == null) {
			...
			if (processor.hasPersistentNBT()) {
				DTCore.LOGGER.warn("Processor Config: '{}' has persistent NBT but is missing the config option.", name);
			}
			return false;
		}
	...
	private static class ProcessorConfig {
		...
		private ProcessorConfig(IDTConfig config, ForgeConfigSpec.Builder builder, INBTProcessor processor) {
